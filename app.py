...
function getWeather(){
    ...
    .catch(()=>alert('Weather API not available'));
}
...
<body>
<h1 id="title">PARTH'S KISAN SAATHI - Dashboard</h1>
<button class="tool-btn" id="weather" onclick="getWeather()">🌤 Weather Info</button>
<button class="tool-btn" id="crop" onclick="getCropAdvice()">🌱 Crop Advice</button>
<button class="tool-btn" id="bill" onclick="alert('Bill Maker coming soon / बिल बनाने वाला जल्द आ रहा है')">🧾 Bill Maker</button>
<button class="tool-btn" id="voice" onclick="voiceCommand()">🎤 Voice Commands</button>
<button class="lang-btn" onclick="setLanguage('en')">English</button>
<button class="lang-btn" style="right:120px;" onclick="setLanguage('hi')">हिंदी</button>
</body>
...
def weather_api():
    # Example static data (future: connect real weather API)
    data = {
        "temp": 30,
        "condition": "Sunny / धूप"
    }
...
@app.route("/crop-api")
def crop_api():
    # Example static advice (future: connect database)
    advice = "Recommended Crops: Wheat, Rice, Maize. Irrigation needed twice a week / सुझावित फसलें: गेहूं, धान, मक्का। सप्ताह में दो बार सिंचाई करें।"
