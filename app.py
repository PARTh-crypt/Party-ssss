...
function voiceAssistant(){
    let cmd = prompt("Enter your command / अपना कमांड लिखें");
    if(!cmd) return;
    fetch('/voice-api',{
        method:'POST',
...
.enter-btn{
  ...
  background:linear-gradient(45deg,#fdd835,#fbc02d);color:#2f6f2f;font-weight:bold;
...
body {
  ...
  background: linear-gradient(135deg,#d0f0c0,#f0fff0,#e0f7fa);
...
h1{font-size:36px;color:#2f6f2f; animation:fadeIn 2s;}
@keyframes fadeIn { from {opacity:0;} to {opacity:1;} }

button{
  margin:12px; padding:14px 32px; border-radius:20px; border:none;
  font-size:18px; color:#fff; cursor:pointer; font-weight:bold; transition:0.3s;
}
button:hover{transform:scale(1.05);}
#weather-btn{background:linear-gradient(45deg,#4fc3f7,#0288d1);}
#crop-btn{background:linear-gradient(45deg,#aed581,#7cb342);}
...
<style>
body {
    margin:0; height:100vh; display:flex; justify-content:center; align-items:center; flex-direction:column;
    font-family:Arial,sans-serif; text-align:center;
    background: linear-gradient(135deg,#c8facc,#a8e063,#87ceeb,#fff176);
    animation: bgShift 20s ease infinite;
}
...
button{
    padding:14px 50px; border-radius:30px; border:none;
    background:linear-gradient(45deg,#fdd835,#fbc02d);
    color:#2f6f2f;font-weight:bold;cursor:pointer;transition:0.3s;
}
...
<h1>PARTH'S KISAN SAATHI</h1>
<h2>Sabka Smart Saathi</h2>
<button onclick="location.href='/dashboard'">Enter App / ऐप में प्रवेश करें</button>
</body>
</html>
""")
...
body{
    font-family:Arial,sans-serif;text-align:center;padding:50px;
    background: linear-gradient(135deg,#a8e063,#c8facc,#87ceeb,#fff176);
    background-size: 400% 400%;
...
h1{font-size:36px;color:#2f6f2f;margin-bottom:40px;}
button{
    margin:12px; padding:14px 32px; border-radius:20px; border:none;
    font-size:18px;color:#fff; cursor:pointer; font-weight:bold; transition:0.3s;
}
